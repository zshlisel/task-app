...
@yeedle yeedle revised this gist 46 minutes ago.
 1 changed file with 18 additions and 0 deletions.
...
@yeedle yeedle revised this gist 1 hour ago.
...
async function main() {
  let response = await fetch('http://localhost:3000/tasks')
@@ -15,18 +16,22 @@ async function addTaskBtnClicked() {
  let newTaskValue = inputElement.value;
  ...
  const result = await response.json()
  console.log(result)
@@ -46,7 +51,7 @@ function displayTasks() {
  for (let i = 0; i < tasks.length; i++){
    let task = tasks[i];
...
@@ -55,9 +60,12 @@ function displayTasks() {
  }
}
...
@yeedle yeedle created this gist 1 hour ago.
